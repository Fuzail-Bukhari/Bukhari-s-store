...
export default function ProductDetail() {
  ...
  const handleAddToCart = () => {
    ...
    toast.success(`${product.name} (${selectedSize}) × ${quantity} added!`);
  };

  const thumbnails = product
    ? [product.image, ...extraImages].filter(Boolean).slice(0, 4)
    : [];
  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-gray-400 font-semibold text-sm">Loading product...</p>
    </div>
  );
  ...
  if (error || !product) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4 px-6">
      ...
      <p className="font-black text-2xl text-black">Product not found</p>
      <p className="text-gray-400 text-sm text-center">
        This product may have been removed or the link is incorrect.
      </p>
      <Link to="/"
        className="bg-black text-white px-8 py-3 rounded-full font-black uppercase tracking-wider hover:bg-gray-800 transition">
        ← Back to Store
      </Link>
    </div>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative overflow-hidden rounded-2xl border-2 border-gray-100 bg-gray-50 aspect-square">
              ...
              <span className={`absolute top-4 right-4 text-xs font-black px-3 py-1.5 rounded-full flex items-center gap-1 ${
                countInStock > 0
                  ? "bg-white border-2 border-black text-black"
                  : "bg-black text-white"
              }`}>
                {countInStock > 0 ? <><CheckCircle size={12}/> In Stock</> : "Out of Stock"}
              </span>
            </div>
              ...
              {thumbnails.map((img, i) => (
                <button key={i} onClick={() => setMainImage(img)}
                  ...
                  <img
                    src={img}
                    alt={`view-${i}`}
                    onError={e => { e.target.onerror = null; e.target.src = "https://placehold.co/200x200?text=Img"; }}
          ...
          <div className="flex flex-col">
            ...
            <div className="bg-black text-white rounded-2xl px-6 py-5 mb-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-black">Rs. {price.toLocaleString()}</p>
              </div>
              <div className="text-right">
                ...
                <p className="text-white font-bold flex items-center gap-1 justify-end">
                  <Package size={14}/> {countInStock} left
                </p>
              </div>
            </div>
            ...
            {product.description && (
              <div className="mb-6">
                <h3 className="font-black text-black text-sm uppercase tracking-widest mb-2">
                  Description
                </h3>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {selectedSize && (
                  <span className="text-xs text-gray-400 font-semibold">
                    Selected: <span className="text-black font-black">{selectedSize}</span>
                  </span>
                )}
              </div>
            ...
            {/* Quantity */}
            <div className="mb-8">
              <h3 className="font-black text-black text-sm uppercase tracking-widest mb-3">
                Quantity
              </h3>
              <div className="flex items-center gap-4">
                <div className="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden">
                  <button onClick={() => setQuantity(q => Math.max(1, q - 1))}
                    className="px-4 py-3 text-black font-black hover:bg-gray-100 transition text-lg">−</button>
                  <span className="px-5 py-3 font-black text-black text-lg border-x-2 border-gray-200 min-w-[60px] text-center">
                    {quantity}
                  </span>
                  <button onClick={() => setQuantity(q => Math.min(countInStock || 10, q + 1))}
                    className="px-4 py-3 text-black font-black hover:bg-gray-100 transition text-lg">+</button>
                </div>
                <p className="text-gray-400 text-sm">
                  Total: <span className="text-black font-black">Rs. {(price * quantity).toLocaleString()}</span>
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mt-6">
              {[
                ...
                <div key={f.title}
                  className="border-2 border-gray-100 rounded-xl p-3 text-center hover:border-black transition">
